    ...
    if(enemy.atkState == 2){
        for(int i = 0; i < EBULLETCOUNT; i++){
            if(!ebullets[i].active){
                ...
                if(count == 0){
                    ebullets[i].row = POS_TO_SCRN_Y(player.pos.y) + (2 * r) + 8;
                    ebullets[i].col = POS_TO_SCRN_X(player.pos.x) + (0 * r) + 8;
                } if(count == 1){
                    ebullets[i].row = POS_TO_SCRN_Y(player.pos.y) + (1 * r) + 8;
                    ebullets[i].col = POS_TO_SCRN_X(player.pos.x) + (1 * r) + 8;
                } if(count == 2){
                    ebullets[i].row = POS_TO_SCRN_Y(player.pos.y) + (0 * r) + 8;
                    ebullets[i].col = POS_TO_SCRN_X(player.pos.x) + (2 * r) + 8;
                } if(count == 3){
                    ebullets[i].row = POS_TO_SCRN_Y(player.pos.y) + (-1 * r) + 8;
                    ebullets[i].col = POS_TO_SCRN_X(player.pos.x) + (1 * r) + 8;
                } if(count == 4){
        ...
        if(pbullets[i].active){
            ...
            if(pbullets[i].row + pbullets[i].rdel < SCREENHEIGHT &&
                ...
                if(collision(pbullets[i].row, pbullets[i].col, pbullets[i].height,
                    ...
                    // for(int j = 0; j <= i; j++) {
                    //     //clearPBullet(&pbullets[j]);
                    //     //pbullets[j].active = 0;
                    //     pbullets[j].hide = 1;
                    // }
                    for(int k = 0; k < EBULLETCOUNT; k++){
                        //clearEBullet(&ebullets[k]);
                        ebullets[k].hide = 1;
                        //drawEBullets();
                        //clearEBullet(&ebullets[k]);
                    }
    ...
    for(int i = 0; i < EBULLETCOUNT; i++){
        ...
        if(ebullets[i].active){
            ...
            if(ebullets[i].row + ebullets[i].rdel < SCREENHEIGHT &&
                ...
                //go to player
                // if(bullets[i].row < enemy.screenRow) bullets[i].row += 1;
                // if(bullets[i].row > enemy.screenRow) bullets[i].row -= 1;
                // if(bullets[i].col < enemy.screenCol) bullets[i].col += 1;
                // if(bullets[i].col > enemy.screenCol) bullets[i].col -= 1;
                ...
                if(collision(ebullets[i].row, ebullets[i].col,
                    ebullets[i].height, ebullets[i].width, POS_TO_SCRN_Y(player.pos.y),
                    POS_TO_SCRN_X(player.pos.x), player.height, player.width)){
                    player.health -= 1;
                    ebullets[i].active = 0;
                    ebullets[i].hide = 1;
                    //testvar = 1;
    ...
    for(int i = 0; i < PBULLETCOUNT; i++){
        if(pbullets[i].active){
            if (pbullets[i].hide) {
                ...
                shadowOAM[52 + i].attr1 = (COLMASK & pbullets[i].col) | ATTR1_SMALL;
                shadowOAM[52 + i].attr2 = ATTR2_PALROW(0) |
                    ATTR2_TILEID(pbullets[i].curFrame * 2, pbullets[i].aniState * 2 + 5);
            }
        }
    }
}
    ...
    for(int i = 0; i < EBULLETCOUNT; i++){
        ...
        if(ebullets[i].active){
            if (ebullets[i].hide) {
                ...
                shadowOAM[2 + i].attr1 = (COLMASK & ebullets[i].col) | ATTR1_SMALL;
                shadowOAM[2 + i].attr2 = ATTR2_PALROW(0) |
                    ATTR2_TILEID(ebullets[i].curFrame * 2, ebullets[i].aniState * 2 + 4);
            }
        }
    }
}