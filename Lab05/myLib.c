...
void setPixel4(int row, int col, unsigned char colorIndex) {
    ...
    unsigned short pixelData = OFFSET(row, col, SCREENWIDTH) / 2;
    if(col & 1){
        videoBuffer[pixelData] = (videoBuffer[pixelData] & 0x00FF) |
            (colorIndex << 8);
    }else{
        videoBuffer[pixelData] = (videoBuffer[pixelData] & 0xFF00) |
            colorIndex;
    }
}
...
void drawRect4(int row, int col, int height, int width, volatile unsigned char colorIndex) {
    ...
    if((width & 1) && (col & 1)){ //odd column and odd with
        for(int r = 0; r < height; r++) {
            setPixel4(row + r, col, colorIndex);
            DMANow(3, &color, &videoBuffer[OFFSET(row + r, col + 1,
                SCREENWIDTH) / 2], DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | (width / 2));
        }
    } if ((width & 1) && !(col & 1)){ //odd width, even col
        for(int r = 0; r < height; r++) {
            DMANow(3, &color, &videoBuffer[OFFSET(row + r, col,
                SCREENWIDTH) / 2], DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | ((width - 1) / 2));
            setPixel4(row + r, col + width, colorIndex);
        }
    } if(!(width & 1) && (col & 1)){ //odd col, even width
        for(int r = 0; r < height; r++) {
            setPixel4(row + r, col, colorIndex);
            DMANow(3, &color, &videoBuffer[OFFSET(row + r, col + 1,
                SCREENWIDTH )/ 2], DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | ((width - 2) / 2 ));
            setPixel4(row + r, col + width, colorIndex);
        }
    } else{ //even width and col
        for(int r = 0; r < height; r++) {
            DMANow(3, &color, &videoBuffer[OFFSET(row + r, col,
                SCREENWIDTH) / 2], DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT |( width / 2));
        }
...
void drawImage3(int row, int col, int height, int width, const unsigned short *image) {
    ...
    for(int r = 0; r < height; r++) {
        DMANow(3, &image[OFFSET(r, 0, width)],
            &videoBuffer[OFFSET(row+r, col, SCREENWIDTH)], width);
    }
}
